...
*/
if(!function_exists('theclick_portfolio_info')){
	function theclick_portfolio_info($args = []){
		if(!is_singular('ef5_portfolio')) return;
		$args = wp_parse_args($args, [
			'title' => esc_html__('Project Description','theclick')
		]);
		// Client name
		$client = theclick_get_post_format_value('portfolio_client_name','Magdalen School');
		// Location
		$location = theclick_get_post_format_value('portfolio_location','Oslay, Canada');
		// Surface Area
		$surface_area = theclick_get_post_format_value('portfolio_surface_area','3,000,000 m2');
		// Budget
		$budgets = theclick_get_post_format_value('portfolio_budgets','$5,000,000.00');
		// Date
		$date = theclick_get_post_format_value('portfolio_complete','');
		$date = strtotime($date);
		...
		<div class="pf-sidebar-box detail-box">
			...
			<div class="inner detail-inner ef5-box-shadow ef5-bg-white transition">
				<ul class="pf-details-list ef5-unstyled">
					<li>
						<strong class="ef5-heading"><?php esc_html_e('Client','theclick'); ?>:</strong>
						<?php echo esc_html($client); ?>
					</li>
					<li>
						<strong class="ef5-heading"><?php esc_html_e('Location','theclick'); ?>:</strong>
						<?php echo esc_html($location); ?>
					</li>
					<li>
						<strong class="ef5-heading"><?php esc_html_e('Surface Area','theclick'); ?>:</strong>
						<?php echo esc_html($surface_area); ?>
					</li>
					<li>
						<strong class="ef5-heading"><?php esc_html_e('Budgets','theclick'); ?>:</strong>
						<?php echo esc_html($budgets); ?>
					</li>
					<li>
						<strong class="ef5-heading"><?php esc_html_e('Complete','theclick'); ?>:</strong>
						<?php echo esc_html($date); ?>
					</li>
					<li>
						<strong class="ef5-heading"><?php esc_html_e('Categories','theclick'); ?>:</strong>
...
*/
if(!function_exists('theclick_portfolio_featured')){
	function theclick_portfolio_featured($args = []){
		$args = wp_parse_args($args, [
			'title'          => esc_html__('Featured Projects','theclick'),
			'posts_per_page' => '3'
		]);
 ...
 * @param $user_ID
*/
if(!function_exists('theclick_portfolio_supporter')){
	function theclick_portfolio_supporter($args = []){
		$args = wp_parse_args($args, [
 ...
 * @param $user_ID
*/
if(!function_exists('theclick_portfolio_attachment')){
	function theclick_portfolio_attachment($args = []){
		$args = wp_parse_args($args, [
			'title' => esc_html__('Download','theclick'),
			'class' => ''
		]);
		$number_of_att = apply_filters('theclick_number_of_portfolio_attachment', 5);
		$files = [];
