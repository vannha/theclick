...
if(!function_exists('theclick_instagram_html_output')){
    ...
    function theclick_instagram_html_output($args=[]){
        ...
        switch ($layout_mode) {
            default:
                echo '<div class="ef5-instagram layout'.$layout_mode.' span'.$span.'">'; 
                    ?>
                ...
                if ($show_author) {
                    ?><div class="user">
                        <a href="//instagram.com/<?php echo trim($username); ?>" target="<?php echo esc_attr( $target ); ?>"><?php if(!empty($author_text)) echo '<span class="author-text">'.esc_html($author_text).'</span>'; echo '<span class="author-name">@'. trim($username).'</span>'; ?></a></div><?php
                }
                echo '</div>';
                break;
